...
if out is None or out.empty:
    ...
    with left:
        # === ONLY COSMETICS CHANGE: charts side-by-side ===
        ...
        with c2:
            ...
            cp_long = show[["strike", "call_gex", "put_gex"]].copy().melt(
                id_vars="strike",
                value_vars=["call_gex", "put_gex"],
                var_name="type",
                value_name="gex",
            )
            cp_long["type"] = cp_long["type"].map({"call_gex": "Call GEX", "put_gex": "Put GEX"})
            ...
            bars = (
                ...
                .encode(
                    ...
                    color=alt.Color(
                        "type:N",
                        scale=alt.Scale(domain=["Call GEX", "Put GEX"], range=["#1f77b4", "#ff7f0e"]),
                        legend=alt.Legend(orient="bottom"),
                    ),
                    tooltip=["strike:O", "type:N", alt.Tooltip("gex:Q", format=",.4f")],
                )
            )
            zero_line = alt.Chart(pd.DataFrame({"y": [0]})).mark_rule(color="gray").encode(y="y:Q")

            st.altair_chart((bars + zero_line).properties(height=320), use_container_width=True)
